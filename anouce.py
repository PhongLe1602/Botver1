...
# Get all announcements from qldt.ptit page including title
# and link to view details of that announcement

import requests
from bs4 import BeautifulSoup
from lxml import html
#The two main libraries used are request and bs4 which are used to access
# and cast the html data type and extract

# Private account to login

login_url = 'https://qldt.ptit.edu.vn/default.aspx?page=dangnhap'
# Link login qldt.ptit

url = 'https://qldt.ptit.edu.vn/default.aspx?page=danhsachthongtin&type=0'
# Link to see all announcements
...
payload = {
    ...
    'ctl00$ContentPlaceHolder1$ctl00$txtTaiKhoa': "",
    'ctl00$ContentPlaceHolder1$ctl00$txtMatKhau': "",
    'ctl00$ContentPlaceHolder1$ctl00$btnDangNhap': 'Đăng Nhập'
}
# Fake login to access qldt.ptit
def showw(pay):
    ans = ""
    with requests.Session() as s:
        # Catch error when accessing
        response = s.post(login_url, data=pay)
        # Fake login

        r = s.get(url)
        # Get data from binary url

        datat = BeautifulSoup(r.text, 'html.parser')
        # Convert text to html

        data_frm = datat.find(id="aspnetForm")
        data_tb = data_frm.find("table", class_="classTable")
        data_tr = data_tb.find_all("tr")
        # # Extract and get the necessary data (title and link)
        #
        li = list()

        for row in range(2, 12):
            td = data_tr[row].find_all("td")
            td_href = td[0].find("a").get('href')
            td_tb = td[0].text

            ###################
            str1 = td_tb[:len(td_tb) - 16]
            str2 = td_tb[len(td_tb) - 11:len(td_tb) - 1] + "."
            arr = list(str1.split())
            str1_1 = ""
            for i in arr:
                str1_1 += i
            ...
            str1_1 = str1_1.strip()
            str3 = str1_1 + " kể từ ngày " + str2
            str4 = str3[:10]

            notice = '-Thông báo: '
            if (str4.lower() != 'thông báo:'):
                str3 = notice + str3
            anoun = f"{str3} \n-Chi tiết tại: {'https://qldt.ptit.edu.vn/' + td_href}\n\n"
            li.append(anoun)
        aa = li[::-1]
        for i in aa:
            ans += i
    return ans
        # Final data to be retrieved
        ##################
        #Standardize and add to the file
